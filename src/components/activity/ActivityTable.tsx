...
interface ActivityTableProps {
  ...
  setBatchAuthStartInput: (_val: string) => void
  batchAuthExpiryInput: string
  setBatchAuthExpiryInput: (_val: string) => void
  batchActivityStartInput: string
  setBatchActivityStartInput: (_val: string) => void
  handleBatchSaveField: (_field: 'start' | 'expiry' | 'activityStart', value: string) => void
  handleToggleAllActions: () => void
  editingCell: { authId: number; field: 'code' | 'start' | 'expiry' | 'activityStart' } | null
  setEditingCell: (_val: { authId: number; field: 'code' | 'start' | 'expiry' | 'activityStart' } | null) => void
  editValue: string
  setEditValue: (_val: string) => void
  handleSaveCell: (_authId: number, field: 'code' | 'start' | 'expiry' | 'activityStart', value: string) => void
  modifiedCells: Record<string, boolean>
  observationCounts: Record<number, number>
  handleOpenObservationsModal: (_row: RcmActivity) => void
  getDenialDescription: (_code: string) => string
...
interface BatchHeaderProps {
  ...
  setActivePopover: (_field: 'start' | 'expiry' | 'activityStart' | null) => void
  inputValue: string
  setInputValue: (_val: string) => void
  onSave: (_field: 'start' | 'expiry' | 'activityStart', value: string) => void
}
...
function ActivityTable({
  ...
  const renderRaActionsCell = (
    row: RcmActivity, authId: number, raStatus: string
  ) => {
          ...
          {isRaError && !raErrorExpanded[authId] ? null : (
            <>
              ...
              <Button
                ...
                style={{ width: '24px', height: '24px', padding: 0, minHeight: '0' }}
    ...
    if (isRaError) {
      ...
      return (
        <Button
          ...
          onClick={() => setRaErrorExpanded((prev) => ({ ...prev, [authId]: !isExpanded }))}
          style={{ height: '24px', padding: '0 6px', minHeight: '0' }}
          leftSection={isExpanded ? <ChevronDown style={{ width: 12, height: 12 }} /> : <ChevronRight style={{ width: 12, height: 12 }} />}
        >Actions</Button>
      )
    }
    let badgeColor = 'blue'
  ...
  const renderRaBillingSummary = (billingSummary: string, row: RcmActivity) => {
    ...
    if (isRepeatTrackerRow && loadingRepeatTracker) {
      return (
        <Group gap="xs" align="center" wrap="nowrap" style={{ height: '24px' }}>
          <Loader size={10} />
          <Text size="sm" color="dimmed" style={{ fontStyle: 'italic', fontSize: 'var(--mantine-font-size-xs)' }}>Analyzing...</Text>
        </Group>
      )
    }
    if (isRepeatTrackerRow && !repeatTrackerLoaded) {
      return (
        <Text size="sm" color="dimmed" style={{
          fontStyle: 'italic', whiteSpace: 'nowrap', fontSize: 'var(--mantine-font-size-xs)', verticalAlign: 'middle'
        }}>Repeat Tracker not loaded.</Text>
      )
    }
    return ''
  }
  ...
  const renderActivityRow = (row: RcmActivity, idx: number) => {
    ...
    const rowBg = isDenied
      ...
      : isPartial
        ...
        : undefined
    const badgeStyle = getBadgeStatusStyle(mappedStatus)
      ...
      >
        ...
        {dateEditMode && (
          <>
            {(
              [
                { field: 'start', value: row.prior_auth_date_time || row.auth_item_date || '', label: '[Add Start]' },
                { field: 'expiry', value: row.auth_item_date || '', label: '[Add Expiry]' },
                { field: 'activityStart', value: (row as any).activity_start_date_time || '', label: '[Add Activity Start]' }
              ] as const
            ).map(({ field, value, label }) => (
              <Table.Td key={field} style={{ padding: '8px 12px', fontSize: 'var(--mantine-font-size-sm)', whiteSpace: 'nowrap' }}>
                {editingCell?.authId === authId && editingCell.field === field ? (
                  <TextInput
                    ...
                    autoFocus placeholder="DD/MM/YYYY HH:MM"
                    styles={{ input: { height: '24px', width: '110px', fontSize: 'var(--mantine-font-size-sm)' } }}
                  />
                ) : (
                  <div
                    onDoubleClick={() => { setEditingCell({ authId, field }); setEditValue(value) }}
        ...
        <Table.Td style={{ padding: '8px 12px', fontSize: 'var(--mantine-font-size-sm)', whiteSpace: 'nowrap' }}>
          {(() => {
            ...
            return <CellText text={String(qtyVal || '')} style={qtyVal > 1 ? { fontWeight: 700 } : undefined} />
          })()}
        </Table.Td>
        <Table.Td style={{ padding: '8px 12px', fontSize: 'var(--mantine-font-size-sm)', textAlign: 'center', whiteSpace: 'nowrap' }}>
          <LtTooltip label="Clinical Evidence Observations & Attachments">
            <Button
              ...
              style={{ height: '24px', padding: '0 8px', minWidth: '24px', fontSize: 'var(--mantine-font-size-xs)', minHeight: '0' }}
        ...
        <Table.Td style={{ padding: '8px 12px', fontSize: 'var(--mantine-font-size-sm)', fontWeight: 600, whiteSpace: 'nowrap' }}>
        ...
        <Table.Td style={{ padding: '8px 12px', fontSize: 'var(--mantine-font-size-sm)', whiteSpace: 'nowrap' }}>
          {(() => {
            ...
            const isPayerRed = (parseFloat(raPayerStr) || 0) <= 0 && (parseFloat(rcmStrVal(row.claim_payer_pay) || '') || 0) > 0
            return <CellText text={raPayerStr} style={isPayerRed ? { color: 'var(--bad, #e03131)', fontWeight: 600 } : undefined} />
          })()}
        </Table.Td>
        <Table.Td style={{ padding: '8px 12px', fontSize: 'var(--mantine-font-size-sm)', fontWeight: 600, whiteSpace: 'nowrap' }}>
        ...
        <Table.Td style={{ padding: '8px 12px', fontSize: 'var(--mantine-font-size-sm)', whiteSpace: 'nowrap' }}>
          {(() => {
            ...
            return <CellText text={rejStr} style={(parseFloat(rejStr) || 0) > 0 ? { color: 'var(--bad, #e03131)', fontWeight: 600 } : undefined} />
          })()}
        </Table.Td>
        <Table.Td style={{ padding: '8px 12px', fontSize: 'var(--mantine-font-size-sm)', whiteSpace: 'nowrap' }}>
          {(() => {
        ...
        <Table.Td style={{ padding: '8px 12px', fontSize: 'var(--mantine-font-size-sm)', whiteSpace: 'nowrap' }}>
          {renderRaActionsCell(row, authId, raStatus)}
        </Table.Td>
        <Table.Td style={{ padding: '8px 12px', fontSize: 'var(--mantine-font-size-sm)', whiteSpace: 'nowrap' }}>
          {renderRaBillingSummary(billingSummary, row)}
        </Table.Td>
      </Table.Tr>
    )
  }
